...
public class BookDAOImpl extends BaseDAO implements BookDAO {
    ...
    public void save(BookInfo bookInfo) {
        String sql = "insert into " + SysConstants.TABLE_BOOK + " values(default, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
        ...
        int count = executeUpdate(
                ...
                bookInfo.getSummary(),
                bookInfo.getContentIntro(),
                bookInfo.getCoverPath(),
                bookInfo.getPrice(),
                bookInfo.getStoreCount(),
                formatter.format(bookInfo.getRegDate()),
                bookInfo.getMemo()
        );
        if(count == 0){
            //可以进行日志记录
            throw new RuntimeException(bookInfo.getBookName() + "添加失败，请检查填写的数据或sql语句！");
        }
    }
    ...
    public List<BookInfo> search(BookSearchCondition condition) {
        ...
        try {
            ...
            //去掉最后多余的or
            while(sql.toString().trim().endsWith("or")){
