        ...
        try {
            ...
            for(Method method : methods){
                ...
                if("set".equals(methodName.substring(0, 3))){
                    ...
                    }else if("LocalDate".equals(paramType) || "java.time.LocalDate".equals(paramType)){
                        //这里需要将java.sql.Date类型转换为java.time.LocalDate
                        Date regDate = resultSet.getDate(filedName);
                        if(null == regDate) continue;
                        //method.invoke(entityObj, regDate.toLocalDate());
                        method.invoke(entityObj, DateUtil.DateToLocalDate(regDate));
                    }else if("LocalDateTime".equals(paramType) || "java.time.LocalDateTime".equals(paramType)){
                        //这里需要将java.sql.Date类型转换为java.time.LocalDateTime
                        Date regDate = resultSet.getDate(filedName);
                        if(null == regDate) continue;
                        method.invoke(entityObj, DateUtil.DateToLocalDateTime(regDate));
                    }else {
